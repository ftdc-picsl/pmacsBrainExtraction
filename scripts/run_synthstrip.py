...
parser = argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter,
                                 prog="synthstrip brain extraction", add_help = False, description='''
...
for input_anatomical in anatomical_images:
    ...
    # Output mask path relative to output data set
    ...
    # Now call synthstrip
    subprocess.run(['singularity', 'run', '--cleanenv', '-B',
                    f"{os.path.realpath(input_dataset_dir)}:/input,{os.path.realpath(output_dataset_dir)}:/output",
                    container, '-i', f"/input/{input_anatomical}", "-m", f"/output/{output_mask}"], env = singularity_env)
